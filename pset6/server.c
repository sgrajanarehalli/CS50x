...
char* indexes(const char* path)
{
    // initialize pointers to store sprintf formatted strings
    char* html_path= malloc(sizeof(char)*(strlen(path)+12));
    char* php_path=malloc(sizeof(char)*(strlen(path)+11));
    
    //use sprintf to combine path to directory with filename
    sprintf(html_path, "%s/index.html",path);
    
    //Attempt to open index file
    if (fopen(html_path, "r"))
    {
        
        return html_path;
    }
    
    //use sprintf to combine path directory with filename
    sprintf(php_path, "%s/index.php",path);
...
bool load(FILE* file, BYTE** content, size_t* length)
{   
    //keeps track of total bytes loaded
    int filesize=0;
    
    //keeps track of bytes currently in buffer
    int bytes_read=0;
    
    //creates buffer and fills it with null bytes
    BYTE buf[BYTES];
    ...
    //attempt to malloc a block of memory
    if (!(*content=malloc(BYTES)))
    {
        return false;
    }
    ...
    do 
    {
        ...
        //copy bytes from buffer into *content
        memcpy(*content+filesize, buf,BYTES);
        filesize+=bytes_read;
        
        //Attempt to resize malloc'd block if necessary
        if (bytes_read==BYTES)
        {
            if(!(*content=realloc(*content,filesize+BYTES)))
                {
                    return false;
                }
        }
...
const char* lookup(const char* path)
{
    //use strchr to find start of extension
    char *ext=strchr(path,46);
...
{
    // Declare int values to keep track of position in request line
    int index=0;
    int len_method=0;
    int len_abs_path=0;
    int len_query=0;
    
    //Declare char arrays to store method and http version
    char method[10];
    ...
    //copy method
    for (int i=0; strncmp(&line[i]," ",1)!=0; i++)
    {
    ...
    //move index forward (+1 to count space)
    index+=len_method+1;
    ...
    //copy abs_path
    for (int i=index; strncmp(&line[i],"?",1)!=0&&strncmp(&line[i]," ",1)!=0;i++)
    {
        //if abs_path contains ", return error
        if (strncmp(&line[i],"\"",1)==0)
        {
            error(400);
            return(false);
        }
        abs_path[i-(index)]=line[i];
        len_abs_path++;
    }
    
    //move index forward (+1 for space or ?)
    index+=len_abs_path+1;
    
    // if abs_path doesn't begin with / return error
    if (strncmp(&abs_path[0],"/",1)!=0)
    {
        error(501);
        return false;
    }
    ...
    if (strncmp(&line[index-1],"?",1)==0)
    {
        for (int i=index; strncmp(&line[i]," ",1)!=0; i++)
        {
            //if query contains ", return error
            if (strncmp(&line[i],"\"",1)==0)
            {
                error(400);
                return(false);
            }
            query[i-(index)]=line[i];
            len_query++;
        }
        //move index forward (+1 for space)
        index+=len_query+1;
    }

    //copy http-version
    for (int i=index; strncmp(&line[i],"\r",1)!=0; i++)
    {

        http_version[i-(index)]=line[i];
    }

    //return error if http-version is incorrect
